...
int WriteParamFlash(uint8_t NUM_PARAM, uint8_t VALUE)
{
	uint16_t buf_param[COUNT_PARAM >> 1] = {0};  // Буфер для хранения параметров.  одная ячейка -- два параметра
	//uint16_t buf_mask_address[256] = {0};  // Буфер для хранения параметров
	uint8_t i;
	
	if (ReadParamFlash(NUM_PARAM) == VALUE){ // во флеш уже лежит нужное значение. переписывать его нет необходимости
		return 0;
	}
	
	if (ReadParamFlash(NUM_PARAM) == 255) { 		// если ячейка параметра пустая, то нет смысла очищать всю страницу
		//FLASH_Read(BEGIN_ADDRESS_PARAM+i*2);
		if (NUM_PARAM & 1){		// если нечетный (старший байт)
			buf_param[0] = VALUE << 8;		// заносим значение в старшую часть ячейки буфера 
			buf_param[0] |= 255; 
		}
		else {				// если четный (младший байт)
			buf_param[0] = VALUE;		// заносим значение в младшую часть ячейки буфера 
			buf_param[0] |= 0xff00;		
		}
		FLASH_Unlock();
		
		for (i=0; i<COUNT_PARAM >> 1; i++) {
			FLASH_Write(BEGIN_ADDRESS_PARAM + (NUM_PARAM >> 1) , buf_param[0]);
		}
		FLASH_Lock();
		return 0;
	}
	...
	if (NUM_PARAM & 1){		// если нечетный (старший байт)
		buf_param[NUM_PARAM >> 1] &= 0x00ff;			// очищаем старший байт
		buf_param[NUM_PARAM >> 1] |= VALUE << 8;		// заносим значение в старшую часть ячейки буфера 
	}
	else {				// если четный (младший байт)
		buf_param[NUM_PARAM >> 1] &= 0xff00;			// очищаем старший байт
		buf_param[NUM_PARAM >> 1] |= VALUE;		// заносим значение в младшую часть ячейки буфера 
	}
...
// запись параметра в FLASH-памяти
int Write16ParamFlash(uint8_t NUM_PARAM, uint16_t VALUE)
{
	uint16_t buf_param[COUNT_PARAM] = {0};  // Буфер для хранения параметров. 
	//uint16_t buf_mask_address[256] = {0};  // Буфер для хранения параметров
	uint8_t i;
	
	if (Read16ParamFlash(NUM_PARAM) == VALUE){ // во флеш уже лежит нужное значение. переписывать его нет необходимости
		return 0;
	}
	
	if (Read16ParamFlash(NUM_PARAM) == 0xffff) { 		// если ячейка параметра пустая, то нет смысла очищать всю страницу
		FLASH_Unlock();
	...
	buf_param[NUM_PARAM] = VALUE;		// заносим значение в старшую часть ячейки буфера 
	
	FLASH_Unlock();
	FLASH_ClearPage(BEGIN_ADDRESS_PARAM);
	for (i=0; i<COUNT_PARAM; i++) {
		FLASH_Write(BEGIN_ADDRESS_PARAM+(i<<1), buf_param[i]);
	}
	FLASH_Lock();
	return 0;
}
