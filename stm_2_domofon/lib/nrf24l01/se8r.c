...
unsigned char buf_1[20] = {0};	// Для первого датчика
unsigned char buf_2[20] = {0};	// Для второго датчика
...
void RX_Mode(void)
{
	...
	SPI1_readWriteReg(NRF_WRITE_REG + SETUP_RETR, 0x1f); // 0x1f -- 500us + 86us, 1f retrans...
	SPI1_readWriteReg(NRF_WRITE_REG + RF_CH, 120);
	// Select RF channel 120
	SPI1_readWriteReg(NRF_WRITE_REG + RX_PW_P0, TX_PLOAD_WIDTH);
	SPI1_readWriteReg(NRF_WRITE_REG + RF_SETUP, 0x47);	//0x47 TX_PWR:+5dBm, Datarate:1Mbps,
	SPI1_readWriteReg(NRF_WRITE_REG + SETUP_AW, 0x02); 	// рекомендации форума 
	
	SPI1_readWriteReg(NRF_WRITE_REG + CONFIG, 0x03);
...
void TX_Mode(/*unsigned char * BUF*/)
{
	...
	SPI1_readWriteReg(NRF_WRITE_REG + RF_SETUP, 0x47);	// TX_PWR:+5dBm, Datarate:1Mbps,
	SPI1_readWriteReg(NRF_WRITE_REG + SETUP_AW, 0x02); 	// рекомендации форума 
	
	SPI1_readWriteReg(NRF_WRITE_REG + CONFIG, 0x02);
}
